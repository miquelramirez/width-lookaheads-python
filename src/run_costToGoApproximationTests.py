...
def run_single_run(iw_parameters,iw_variants, run_num, Domain, sim_dt, sim_budget, horizon, numberRollouts, runNum) :
    import time
    import numpy as np
    import random
    import json
...
def run_experiment(Domain, sim_dt, sim_budget, horizon, numberRollouts, N) :
    ...
    iw_variants = {'Name': ["iw_depth_random_rollout"]}#, "iw_depth_knuth"]}#, "iw_depth_heur"]}#, "iw_depth_stochastic_enum"]}
    iw_parameters = [iw_depth_random_rollout_parameters]#, iw_depth_knuth_parameters]#, iw_depth_heur_parameters]#, iw_depth_stochastic_enum_parameters]
...
def main(cmd_line_args) :
    ...
    for intState in range(1):
        ...
        for Domain, H, numberRollouts in [("GridWorld-10x10-initS{}-v{}".format(intState, gridVersion), 50, 10)]:
